...
ggplot() +
  ...
  geom_sf(data=metroStops, 
          ...
          color = "#800000")+ # str_split(LINE,",",simplify=T)[1]) maybe (too many legend entries)
  geom_sf(data=metroLines, 
          aes(colour=NAME, size=NAME))+ # Why is color wrong, but it gets the right names?
  scale_colour_manual(values=c("blue","green","orange","red","gray","yellow"))+
  scale_size_manual(values = c(5, 5, 5, 5, 5, 5)) +  # Adjust line width as needed
