...
# alternate if you want to show all Population plots & note NA values in X (or remove them)
pop_rent2009 <- TOD_tracts %>%
  st_drop_geometry() %>%
  group_by(ID) %>%
  summarize(x = mean(MedRent), y = sum(TotalPop),removed_x=!complete.cases(mean(MedRent)))
...
ggplot() +
 HEAD
  geom_sf(data = tracts09, fill = "grey80") +
...
geom_sf(data = allTracts.group,
  ...
  geom_sf(data = labels_y,
          ...
          fill = alpha("blue", 0.7),
          col = "gray30") +
 78e701996c71ef7a67185fa565ade6df52d28146
  scale_size(
    range = c(.5, 6),
    guide = guide_legend(
...
ggplot() +
  geom_sf(data = allTracts.group, fill = "#DFED6B") +
  geom_sf(data = labels_y,
          pch = 21,
          aes(size = x),
          fill = alpha("#0868ac", 0.8),
          col = "black") +
  scale_size(
    range = c(.5, 6),
    guide = guide_legend(
      ...
      label.position = "bottom")) +
  guides(fill = guide_legend(title = "")) +
  labs(title = "Median Rent Near DC Metro stations, 2009",
       sub = "Based on 1/2 mile buffers",
       size = "") +
  #theme_void() +
  theme(legend.position = "bottom") +
  labs(caption = "Figure x.x DC Median Rent") +
  facet_wrap(~year) +
...
```
# Adusted submarket map colors - Trying to add some transparency to the TOD and non to help the metro lines pop  and I failed..still working on it. 
```{r}

submarket_colors <- c("Non-TOD" = "grey90", "One line" = "orangered3", "Two lines" =  "red", "Three or more lines" = "brown")
...
# I Need to figure out how to change the greay in variable title and how to make sure name does not get cut off
```{r}
submarket_colors_graph <- c("Non-TOD" = "grey24", "One line" = "orange", "Two lines" =  "red", "Three or more lines" = "brown")
st_drop_geometry(allTracts.threeMarkets) %>% #Added a faceted plot for the submarkets feel free to edit
...
#  small multiple plots - adujst for variables

```{r}
  ggplot(allTracts.threeMarkets)+ #plugged in the chunk from lab, adjust for variables 
    geom_sf(data = st_union(allTracts.threeMarkets))+
    geom_sf(aes(fill = q5(pctNonWhite))) +
    ...
    labs(title = "Percent of Non-White Population", subtitle = "2009-2017 DC Metro Area with 1/2 mile station buffers") +
    facet_wrap(~year)+
    mapTheme() + 
    theme(plot.title = element_text(size=22))
...
Dropping a sample code chunk here that fetches/mutates my variables of interest, also calculates ICE scores for TOD areas by year.
...
- ICE_TOD and ICE_tract: racial and economic index of concentration at the extremes
...
1. Rent burden: As rent rises and income remains constant, the percentage of income spent on rent must rise as well. This percentage represents a key factor in whether residents must relocate because of rising rents that become unaffordable. It is most critical for lower income residents because they have less disposable income and limited options for relocation.
...
Some Definitions of housing Variable Concepts [here](https://www.census.gov/housing/hvs/definitions.pdf)
...
Years 2017 & 2020 had the same references for variables, so the jump from 2009 -> the 2017/2020 is where you might want to be careful.

Added in ...

1. Total Number of Reported Public Transportation (not taxicab) commuters to Work - 'Renamed Var Name'
2. Total Number of Public Transit Users with Travel Time < 30 minutes - 'Renamed Var Name'
3. Total Number of Public Transit Users with Travel Time 30<=x<60 minutes - 'Renamed Var Name'
4. Total Number of Public Transit Users with Travel Time >60 minutes - 'Renamed Var Name'

(Also renamed RentBurden in my version to 'RentPortionOfInc' due to retrieving it again before realizing it was RentBurden)

There's a small desire to see if work commute time is a factor in higher median rent for specific tracts/block groups & if the trend changes across years.
- Note to self, changes to variables end by noon Wednesday, 9/21
