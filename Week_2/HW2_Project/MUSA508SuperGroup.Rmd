...
tracts2009 <- 
  ...
  mutate(pctWhite = ifelse(TotalPop > 0, Whites / TotalPop,0),
         ...
         pctTransitCommute = TotalTransitCommuters/WorkTransTotal, # ifelse(WorkTransTotal!=0,TotalTransitCommuters/WorkTransTotal,0)
         pctPubTrans_LT30 = ifelse(TotalTransitCommuters!=0,PubTrans_LT30/TotalTransitCommuters,0),
         pctPubTrans_30To59 = ifelse(TotalTransitCommuters!=0,PubTrans_30To59/TotalTransitCommuters,0),
         pctPubTrans_HourUp = ifelse(TotalTransitCommuters!=0,PubTrans_HourUp/TotalTransitCommuters,0),
         year = "2009") %>%
...
#### Retrieve Metro Data 
```{r pull_metro_data_w3, results='hide'}
metroStops <- st_read("https://maps2.dcgis.dc.gov/dcgis/rest/services/DCGIS_DATA/Transportation_Rail_Bus_WebMercator/MapServer/52/query?outFields=*&where=1%3D1&f=geojson") %>% 
    dplyr::select(NAME, LINE) %>%
...
allTracts.Summary <- #also no idea if this works, just kind of plugged it in from the lab rmd
  st_drop_geometry(allTracts.group) %>%
    ...
    summarize(Rent = mean(MedRent, na.rm = T),
              ...
              Percent_LowTravelTime = mean(pctPubTrans_LT30*100, na.rm = T),
              Percent_HalfToHourTravelTime = mean(pctPubTrans_30To59*100, na.rm = T),
              Percent_HourPlusTravelTime = mean(pctPubTrans_HourUp*100, na.rm = T))
